...
- Le compteur de pommes est porte par `state.applesEaten` dans `src/engine.js`, affiche via `#apples-value`, incremente uniquement quand une pomme est mangee et revient a `0` via `createInitialState`/`resetGame`.
